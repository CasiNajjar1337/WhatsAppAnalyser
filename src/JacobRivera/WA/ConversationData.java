...
public class ConversationData {
    ...
    private Map<String, Integer> messagesMap = new HashMap<String, Integer>();
    ...
    public void addData(String participant, String message, Date date, String time) {
        ...
        Integer numMessCount = messagesMap.get(message);
        messagesMap.put(message, (numMessCount == null) ? 1 : numMessCount + 1);
    ...
    public int getTotalParticipants() {
        int total = 0;
        for (String iterator : messagesMap.keySet())
            total = total + messagesMap.get(iterator);
        return total;
    }
    ...
    public float getMessagesShare(String pt) {
        float avg;
        int tot = getTotalParticipants();
        avg = (float) (messagesMap.get(pt) * 100.0) / tot;
        return avg;
    }
    ...
    public Dataset getDaysDataSet() {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy");
        for (Date iterator : days.keySet()) {
            dataset.addValue(days.get(iterator), "Nachrichten pro Tag", sdf.format(iterator));
        }
        return dataset;
    }
    ...
    public Dataset getTotalDaysDataSet() {
        ...
        for (Date iterator : totalDays.keySet()) {
            dataset.addValue(totalDays.get(iterator), "Nachrichten pro Tag", sdf.format(iterator));
        }
        return dataset;
    }

    public Iterable<? extends PieChart.Data> getChartMessegesData() {
        List<PieChart.Data> data = new ArrayList<>();
        for (String s : messagesMap.keySet()) {
            data.add(new PieChart.Data(s, getMessagesShare(s)));
        }
        return data;
    }
}
